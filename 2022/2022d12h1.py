    ...
    for _ in range(pocet_riadkov*pocet_stlpcov):
        ...
        for smer in smery:
            ...
            if 0 <= sused[0] < pocet_riadkov and 0 <= sused[1] < pocet_stlpcov:
                if abeceda.index(hory[sused[0]][sused[1]]) - abeceda.index(hory[terajsie[0]][terajsie[1]]) < 2:
                    if sused == koniec:
