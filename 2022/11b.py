...
with open("aoc/input.txt", "r") as subor:
    while True:
        ...
        if riadok != "":
            ...
            vymazavac *= int(opica_na_pridanie["test"])
    ...
    for opica in opice:
        for item in opice[opica]["veci"]:
            ...
            if nove_cislo % int(opice[opica]["test"]) == 0:
                opice[opice[opica]["ak hej"]]["veci"].append(nove_cislo % vymazavac)
            else:
                opice[opice[opica]["ak nie"]]["veci"].append(nove_cislo % vymazavac)
