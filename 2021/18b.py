...
najviac = 0
for cislo in slimacie_cisla:
    for druhe_cislo in slimacie_cisla:
        if cislo != druhe_cislo:
            aktualne_slimacie_cislo = f"[{cislo}, {druhe_cislo}]"
            temp = skontroluj_cislo(aktualne_slimacie_cislo)
            while aktualne_slimacie_cislo != temp:
                aktualne_slimacie_cislo = temp
                temp = skontroluj_cislo(aktualne_slimacie_cislo)

            t = zisti_m(ast.literal_eval(aktualne_slimacie_cislo))
            if t > najviac:
                najviac = t
